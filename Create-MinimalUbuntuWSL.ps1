...
function Set-ClaudeTerminalProfile {
    ...
    try {
        ...
        $autoGeneratedProfile = $settings.profiles.list | Where-Object { 
            $_.source -eq "Windows.Terminal.Wsl" -and 
            ($_.name -eq $InstanceName -or $_.commandline -like "*$InstanceName*")
        }
        ...
        if ($autoGeneratedProfile) {
            ...
            $autoGeneratedProfile.name = "$InstanceName 🤖"
            ...
            $manualProfile = $settings.profiles.list | Where-Object { $_.name -eq "$InstanceName 🤖" }
            if ($manualProfile) {
                # 既存の手動プロファイルを更新
